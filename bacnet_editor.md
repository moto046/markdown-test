...
- CharacterString はエディタでバイト数範囲を 0～127（NULL文字を除く）として設定する
- ただし `Object_Name` は空文字不可のため 1～127 文字で設定する
...
| Property | 対象Object | データ型 | 集合種別 | 初期値 | 設定可能範囲 | バイト数 | 説明 | エディタでの設定内容 | ドライバの対象設定 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Object_Identifier | Device | ObjectIdentifier | 単一値 | 260001 | 0 ~ 4194303 | 4 | デバイス識別子 | 左記の通り。説明の文言でUI表示 | なし |
| APDU_Timeout | Device | Unsigned | 単一値 | 30 | 0 ~ 999 | 8 | タイムアウト(*100ms) | 左記の通り。説明の文言でUI表示 | タイムアウト時間 |
| Number_Of_APDU_Retries | Device | Unsigned | 単一値 | 3 | 0 ~ 255 | 8 | リトライ回数 | 左記の通り。説明の文言でUI表示 | リトライ回数 |
| MAC_Address | NP（BACnet/IP） | OCTET STRING | 単一値 | - | IPv4(4)+UDP Port(2)の6バイト | 6 | IPv4+UDPポート | 別の設定で完了するため、専用の設定は不要 | なし |
| BACnet_IP_Mode | NP（BACnet/IP） | ENUMERATED | 単一値 | 0 | NORMAL(0) / FOREIGN(1) | 4 | IP動作モード | FD設定有無にて判別可能なため、専用の設定は不要 | なし |
| BACnet_IP_UDP_Port | NP（BACnet/IP） | Unsigned | 単一値 | 47808 | 0〜65535 | 8 | ポートNo | 左記の通り。説明の文言でUI表示 | 自局ポートNo. |
| FD_BBMD_Address | NP（BACnet/IP） | 構造体 | 単一値 | IPv4アドレス:0.0.0.0(0)<br> UDPポート:47808 | IPv4アドレス:0.0.0.0 ~ 255.255.255.255(0 ~ 0xFFFFFFFF)<br> UDPポート:1024 ~ 65535 | IPv4アドレス:4 <br> UDPポート:2 | 参照BBMDアドレス | 2つに設定を分けて対応。<br> 1.BBMDアドレス（FD登録先）:IPv4文字列入力<br> 2.BBMDポートNo（FD登録先）:数値入力 <br><br>本設定はFD機能を使用する場合のみ設定可能| なし |
| FD_Subscription_Lifetime | NP（BACnet/IP） | Unsigned | 単一値 | 600 | 0〜65535 | 8 | FD登録TTL（秒） | 左記の通り。説明の文言でUI表示 <br><br>本設定はFD機能を使用する場合のみ設定可能| なし |
...
| ローカル画面切替時の初期化 | 1 | 0 / 1 | 1 | ローカル画面切替時のProperty値の初期化有無。<br>1の場合はローカル画面移行時にPropertyの値を解放する。<br>1の場合は画面データ変更時にPropertyの値を解放する。 | なし |
...
| APDU_Timeout | Device | Unsigned | 単一値 | 30 | 0 ~ 999 | 8 | タイムアウト(*100ms) | 左記の通り。説明の文言でUI表示 | タイムアウト時間 |
| Number_Of_APDU_Retries | Device | Unsigned | 単一値 | 3 | 0 ~ 255 | 8 | リトライ回数 | 左記の通り。説明の文言でUI表示 | リトライ回数 |
| MAC_Address | NP（BACnet/MS/TP） | OCTET STRING | 単一値 | 0 | 0〜127 | 1 | ステーションアドレス | 左記の通り。説明の文言でUI表示 | 自局番 |
| Link_Speed | NP（BACnet/MS/TP） | REAL | 単一値 | 38400 | 9600 / 19200 / 38400 / 57600 / 76800 / 115200 | 4 | 通信速度 | 左記の通り。説明の文言でUI表示 | ボーレート |
| Max_Manager | NP（BACnet/MS/TP） | Unsigned | 単一値 | 127 | 0〜127 | 8 | 最大マスターアドレス | 左記の通り。説明の文言でUI表示 | なし |
| Max_Info_Frames | NP（BACnet/MS/TP） | Unsigned | 単一値 | 1 | 1〜8 | 8 | 1トークンあたりの最大送信フレーム数 | 左記の通り。説明の文言でUI表示 | なし |
...
| Property | Property Identifier | アクセス | データ型 | 集合種別 | 初期値設定要否 | 初期値 | 設定可能範囲 | メモリ設定要否 | メモリ書込可否 | バイト数 | 有効bit数 | 説明 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Object_Identifier | 75 | R | ObjectIdentifier | 単一値 | × | - | - | × | - | 4 | - | オブジェクト識別子 |
| Object_Name | 77 | R | CharacterString | 単一値 | ○ | "ANALOG INPUT <instance>" | 1〜127文字（一意） | × | - | 0～127 | - | オブジェクト名 |
| Object_Type | 79 | R | ENUMERATED | 単一値 | × | - | - | × | - | 4 | - | オブジェクト種別（AI固定） |
| Present_Value | 85 | R | REAL | 単一値 | ○ | 0.0 | 実装上のREAL範囲 | ○ | ○ | 4 | - | 現在値 |
| Description | 28 | R | CharacterString | 単一値 | ○ | "" | 0〜127文字 | × | - | 0～127 | - | 説明文 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Object_Identifier | 75 | R | ObjectIdentifier | 単一値 | × | - | - | × | - | 4 | - | オブジェクト識別子 |
| Object_Name | 77 | R | CharacterString | 単一値 | ○ | "ANALOG OUTPUT <instance>" | 1〜127文字（一意） | × | - | 0～127 | - | オブジェクト名 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Object_Identifier | 75 | R | ObjectIdentifier | 単一値 | × | - | - | × | - | 4 | - | オブジェクト識別子 |
| Object_Name | 77 | R | CharacterString | 単一値 | ○ | "BINARY INPUT <instance>" | 1〜127文字（一意） | × | - | 0～127 | - | オブジェクト名 |
...
| Active_Text | 4 | R | CharacterString | 単一値 | ○ | "Active" | 0〜127文字 | × | - | 0～127 | - | ACTIVE表示文字列 |
| Inactive_Text | 46 | R | CharacterString | 単一値 | ○ | "Inactive" | 0〜127文字 | × | - | 0～127 | - | INACTIVE表示文字列 |
| Property_List | 371 | R | ENUMERATED | Array（可変長） | × | - | - | × | - | 可変 | - | 保持プロパティ一覧 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Object_Identifier | 75 | R | ObjectIdentifier | 単一値 | × | - | - | × | - | 4 | - | オブジェクト識別子 |
| Object_Name | 77 | R | CharacterString | 単一値 | ○ | "BINARY OUTPUT <instance>" | 1〜127文字（一意） | × | - | 0～127 | - | オブジェクト名 |
...
| Object_Name | 77 | R | CharacterString | 単一値 | ○ | "SimpleServer" | 1〜127文字（一意） | × | - | 0～127 | - | デバイス名 |
| Object_Type | 79 | R | ENUMERATED | 単一値 | × | - | - | × | - | 4 | - | オブジェクト種別（Device固定） |
| System_Status | 112 | R | ENUMERATED | 単一値 | × | - | - | × | - | 4 | - | デバイス状態（bacnet-stack管理） |
| Vendor_Name | 121 | R | CharacterString | 単一値 | × | - | - | × | - | 0 ~ 127 | - | ベンダー名 |
| Vendor_Identifier | 120 | R | Unsigned | 単一値 | × | - | - | × | - | 8 | - | ベンダーID |
| Model_Name | 70 | R | CharacterString | 単一値 | × | - | - | × | - | 0 ~ 127 | - | モデル名 |
| Firmware_Revision | 44 | R | CharacterString | 単一値 | × | - | - | × | - | 0 ~ 127 | - | FWリビジョン |
| Application_Software_Version | 12 | R | CharacterString | 単一値 | × | - | - | × | - | 0 ~ 127 | - | アプリバージョン |
...
| Object_Name | 77 | R | CharacterString | 単一値 | ○ | "Network Port" | 1〜127文字（一意） | × | - | 0～127 | - | ポート名 |
